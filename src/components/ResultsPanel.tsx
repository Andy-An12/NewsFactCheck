...
export default function ResultsPanel({ keyword, evidence, factChecks }: Props) {
    return (
        <section style={styles.card}>
            ...
            <h3 style={styles.sectionTitle}>Where the keyword is spoken</h3>
            ...
            )) : <p style={styles.muted}>The keyword was not spoken anywhere in this content. Try a shorter or different keyword.</p>}

            <h3 style={styles.sectionTitle}>Fact-check of what was said</h3>
            {factChecks.length ? factChecks.map((item, i) => (
                <article key={i} style={styles.resultBox}>
                    <div style={styles.factHeader}>
                        <strong>{item.claim}</strong>
                        <span style={{ ...styles.pill, ...verdictStyle(item.verdict) }}>{item.verdict} · {Math.round(item.confidence * 100)}%</span>
                    </div>
            ...
            )) : <p style={styles.muted}>No keyword segments to fact-check — nothing was verified.</p>}
        </section>
    );
}

function verdictStyle(verdict: string): React.CSSProperties {
    if (verdict === 'verified') return { color: '#6ee7b7', borderColor: '#14532d' };
    if (verdict === 'false') return { color: '#fca5a5', borderColor: '#7f1d1d' };
...
const styles = {
    ...
    pill: { background: '#172235', border: '1px solid #27344d', borderRadius: 999, padding: '4px 8px', color: '#c4b5fd', fontSize: 12, whiteSpace: 'nowrap' },
} as const;
